...
async def list_events(
    ...
    events = await db.fetch_many(
        "event_queue", filters=filters, order_by="urgency", ascending=False, limit=limit
    )
...
async def acknowledge_event(event_id: UUID, auth: dict = Depends(verify_auth)):
...
async def dismiss_event(event_id: UUID, auth: dict = Depends(verify_auth)):
    """Dismiss an event"""
    user_id = auth["user_id"]
    ...
    result = await db.update(
        "event_queue", {"status": "dismissed"}, {"id": event_id, "user_id": user_id}
    )
